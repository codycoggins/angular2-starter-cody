import { Component, OnInit, OnChanges, ElementRef, Inject, Directive } from 'angular2/core';
declare var d3, nv: any;

@Component({
  selector: 'nvd3-watson',
  inputs: ['options', 'data'],
  template: ``
})

export class NvD3Watson {
  options: any;
  data: any;
  el: any;
  chart: any;
  ...
  updateWithOptions(options) {
    ...
    nv.addGraph(function() {
      ...
      self.chart.resizeHandler = nv.utils.windowResize(function() {
        let dummy = self.chart && self.chart.update && self.chart.update();
      });
